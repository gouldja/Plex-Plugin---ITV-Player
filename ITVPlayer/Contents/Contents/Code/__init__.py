...
def MainMenu():
    dir = MediaContainer(viewGroup="Menu")  
    dir.Append(Function(DirectoryItem(RenderEpisodeList, title = TOP_TEN_TITLE), url = ITV_API_TOPTEN_URL))
    ...
    dir.Append(Function(DirectoryItem(AddDateList, title = TV_DATE_TITLE)))
    dir.Append(Function(DirectoryItem(AddAToZ, title = A_Z_TITLE))) 
    dir.Append(Function(InputDirectoryItem(SearchResults,SEARCH_TITLE,SEARCH_SUBTITLE,summary=SEARCH_SUMMARY)))
    return dir
...
def AddDateList(sender, query = None, url = None, subtitle = None, sort_list = None, thumb_url = ICON, player_url = ITV_SD_PLAYER_URL):
   ...
   dir.Append(Function(DirectoryItem(RenderEpisodeList, title = "Today"), url = ITV_API_DATE_URL % today.strftime("%Y/%m/%d")))
   date = (today+Datetime.Delta(days = -1))
   dir.Append(Function(DirectoryItem(RenderEpisodeList, title = "Yesterday"), url = ITV_API_DATE_URL % date.strftime("%Y/%m/%d")))
   
   
   for number in range (2, 31):
      date = (today+Datetime.Delta(days = -number))
      dir.Append(Function(DirectoryItem(RenderEpisodeList, title = date.strftime("%a, %d %b %Y")), url = ITV_API_DATE_URL % date.strftime("%Y/%m/%d")))
   return dir
